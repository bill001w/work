    ...
    <div class="content">
      <div class="nav">
        <div class="tree_right">
          <div v-for="(item,index) in htmlNav" v-html="item" :class="{'active':index===currentIdx}" class="titleNav" @click="go(index)"></div>
        </div>
      </div>
      <div id="markdownbody" class="markdown-body grey-scroll" @scroll="scrollList($event)" ref="main">
        <div style=" max-width: 700px; margin: 0 auto;">
          <p style="font-size: 28px; color: #333; padding: 35px 0 30px;" id="articletitle"></p>
          <div v-html="htmlRaw"></div>
          <div :style="{'height':(height-50)+'px'}"></div>
        </div>
      </div>
      <transition name="scroll-top-fade">
        <div class="scroll-top-btn" :class="{'hover': hoverScrollBtn}" v-show="showScrollTopBtn" @mouseenter="hoverScrollBtn = true" @mouseleave="hoverScrollBtn = false" @click="scrollTop">
